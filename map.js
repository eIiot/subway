...
let zoom = d3.zoom().scaleExtent([0.7, 5]).on("zoom", zoomed);
svg.call(zoom);
...
let defaultStop = "EMBR"; // Embarcadero
...
let computeStationPositions = (originStationId, travelTimes) => {
  ...
  for (let stationId of Object.keys(stations)) {
    ...
    let dist = travelTimes
      ? travelTimes[stationId] / maxTravelTime
      : origDist * 5;
    positions[stationId] = {
